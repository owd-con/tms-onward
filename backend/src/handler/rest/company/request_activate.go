...
func (r *activateRequest) execute() (*rest.ResponseBody, error) {
	err := r.uc.Activate(r.company)
	if err != nil {
		return nil, err
	}

	return rest.NewResponseMessage("Company activated successfully"), nil
}

func (r *activateRequest) with(ctx context.Context, uc *usecase.CompanyUsecase) *activateRequest {
	r.ctx = ctx
