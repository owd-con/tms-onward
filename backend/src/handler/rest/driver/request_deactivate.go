...
func (r *deactivateRequest) execute() (*rest.ResponseBody, error) {
	err := r.uc.Driver.Deactivate(r.driver)
	if err != nil {
		return nil, err
	}

	return rest.NewResponseMessage("Driver deactivated successfully"), nil
}

func (r *deactivateRequest) with(ctx context.Context, uc *usecase.Factory) *deactivateRequest {
	r.ctx = ctx
