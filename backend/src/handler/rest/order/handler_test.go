...
func TestHandler_ShowByNumber_Success(t *testing.T) {
	uc := usecase.NewFactory()
	h := &handler{uc: uc}
	...
	// Create an order with unique order number
	testCtx := context.Background()
	orderNumber := "ORD-" + uuid.New().String()
	order := &entity.Order{
		CompanyID:           company.ID,
	...
	restCtx := createTestContext("GET", "/orders/"+orderNumber+"/by-number", nil, user.ID.String(), company.ID.String(), pathParams)

	err := h.showByNumber(restCtx)
