...
func (r *activateRequest) execute() (*rest.ResponseBody, error) {
	err := r.uc.Activate(r.pricingMatrix)
	if err != nil {
		return nil, err
	}

	return rest.NewResponseMessage("Pricing matrix activated successfully"), nil
}

func (r *activateRequest) with(ctx context.Context, uc *usecase.PricingMatrixUsecase) *activateRequest {
	r.ctx = ctx
