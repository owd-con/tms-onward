...
func (r *deactivateRequest) execute() (*rest.ResponseBody, error) {
	err := r.uc.Deactivate(r.vehicle)
	if err != nil {
		return nil, err
	}

	return rest.NewResponseMessage("Vehicle deactivated successfully"), nil
}

func (r *deactivateRequest) with(ctx context.Context, uc *usecase.VehicleUsecase) *deactivateRequest {
	r.ctx = ctx
