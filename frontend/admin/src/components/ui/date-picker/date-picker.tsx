...
export const DatePicker = memo(({
  ...
  const onChangeMonthLeft = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const onChangeYearLeft = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const onChangeMonthRight = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    const monthNumber = Number(e.target.value);
    setViewMonths((prev) => {
  ...
  const onChangeYearRight = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    const yearNumber = Number(e.target.value);
    setViewMonths((prev) => {
      const leftYear = prev[0].year();
  ...
  const monthsToRender = useMemo(() => isRange ? viewMonths : [viewMonths[0]], [isRange, viewMonths]);
