...
export const FailedOrdersAlert = memo<FailedOrdersAlertProps>(
  ({ orders, className }) => {
    ...
    // Calculate total failed shipments across all orders
    const totalFailedShipments = orders.reduce((sum, order) => sum + (order.failed_shipments_count || 0), 0);
      ...
      <div className={className}>
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-lg font-semibold">Failed Shipments</h3>
            <span className="badge badge-error badge-sm">{totalFailedShipments}</span>
            <span className="text-xs text-base-content/60">({orders.length} orders)</span>
          </div>
            ...
            <table className="table table-sm table-zebra w-full">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-3 text-left">Failed Shipments</th>
                  ...
                  <tr key={order.id}>
                    ...
                    <td className="text-xs px-4 py-3">
                      <span className="badge badge-error badge-xs">
                        {order.failed_shipments_count || 0}
                      </span>
                    </td>
