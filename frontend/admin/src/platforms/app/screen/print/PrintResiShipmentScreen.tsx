...
/**
 * TMS Onward - Print Resi Shipment Screen
 *
 * Print resi (surat jalan) untuk 1 shipment tertentu.
 * Layout RESI (102mm x 127mm) dengan 2 bagian (atas & bawah).
 *
 * URL format: /print/resi/order/:orderId/shipment/:shipmentId
 *
 * Menggunakan direct render dengan React components (QRCode, Barcode).
 */
const PrintResiShipmentScreen = () => {
  const pathParts = window.location.pathname.split("/");
  const orderIdIndex = pathParts.indexOf("order") + 1;
  const shipmentIdIndex = pathParts.indexOf("shipment") + 1;

  const orderId = pathParts[orderIdIndex];
  const shipmentId = pathParts[shipmentIdIndex];
  ...
  useEffect(() => {
    if (showOrderResult?.isLoading) return;
    const orderData = (showOrderResult?.data as any)?.data;
    setOrder(orderData);

    if (orderData?.shipments && orderData.shipments.length > 0) {
      const foundShipment = orderData.shipments.find(
        (s: Shipment) => s.id === shipmentId,
      );
      setShipment(foundShipment || null);
    }
  }, [showOrderResult]);

  if (showOrderResult?.isLoading || !order || !shipment) return null;

  return (
    <div>
      <Print
        title={`Resi - ${shipment?.shipment_number || ""}`}
        size='RESI'
            ...
            <div style={{ height: 325 }}>
              ...
              >
                <table className='body border'>
                  ...
                  <tr className='border'>
                    <td className='border' colSpan={2} rowSpan={2} valign='top'>
                      <div style={{ height: 50 }}>
                        <div
              ...
              >
                <table className='body border'>
                  <tr className='border'>
                    ...
                    <td className='border center' width={"39%"} valign='middle'>
                      <div
...
export default PrintResiShipmentScreen;
