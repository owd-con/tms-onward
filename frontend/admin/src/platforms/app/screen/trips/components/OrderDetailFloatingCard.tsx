...
export const OrderDetailFloatingCard: React.FC<
  OrderDetailFloatingCardProps
> = ({ order, onClose, onAssign, onReturn }) => {
  const isException =
    !!order.failed_shipments && order.failed_shipments.length > 0;

  const { data: previewData, isLoading: isLoadingPreview } =
    orderApi.endpoints.getWaypointPreview.useQuery(
      { id: order.id },
      { skip: isException },
    );
    ...
    ? order.failed_shipments!.flatMap((fs: any, idx: number) => [
        {
          type: "pickup",
          location_name:
            fs.origin_location_name || fs.origin_location || "Origin City",
        ...
        },
        {
          type: "delivery",
          location_name:
            fs.dest_location_name || fs.dest_location || "Destination City",
          address:
            fs.dest_address || fs.dest_location || "Address not available",
    ...
    <div className='lg:absolute lg:top-6 lg:left-6 w-full lg:w-[400px] bg-white/95 backdrop-blur-md lg:rounded-[24px] rounded-t-[24px] lg:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] shadow-none border-x border-t lg:border-white/60 border-slate-100 overflow-hidden z-20 flex flex-col h-full lg:h-auto lg:max-h-[calc(100vh-48px)]'>
      ...
      <div className='px-6 pt-5 pb-4 flex justify-between items-center bg-white/50 border-b border-slate-100'>
        <div className='flex items-center gap-3'>
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-[11px] tracking-widest border shrink-0 ${isException ? "bg-red-100/80 text-red-700 border-red-200/50" : badgeColors}`}
          >
            {isException ? "FAILED" : order.order_type}
          </div>
          <div className='flex flex-col min-w-0'>
            <h2 className='text-[15px] font-medium text-slate-900 tracking-tight leading-none mb-1 truncate'>
              {orderNumber}
            </h2>
      ...
      <div className='flex-1 overflow-y-auto px-6 py-4 pb-8 space-y-4'>
        ...
        <div className='p-4 bg-transparent rounded-2xl border border-slate-200 flex gap-4'>
          ...
          <div className='flex-1'>
            ...
            <p
              className={`text-[13px] font-bold ${isException ? "text-red-600" : "text-slate-900"}`}
            >
              {isException
                ? order.failed_shipments?.length
              ...
              {displayWaypoints.map((wp: any, idx: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`absolute -left-2 top-0 w-9 h-9 rounded-xl flex items-center justify-center z-10 bg-${color}-50`}
                    >
                      <MapPin
                        size={18}
                    ...
                    <div className='flex flex-col pt-1'>
                      <div className='flex justify-between items-start gap-4 w-full'>
                        <div className='flex-1 min-w-0'>
                          <h3 className='text-[13px] font-bold text-slate-900 mb-0.5 leading-none'>
                            {wp.location_name}
                          </h3>
                          <p className='text-[11px] text-slate-600 mb-1.5 truncate max-w-full'>
                            {wp.address}
                          </p>
                          {isFailed && (
                            <p className='text-[10px] font-bold text-red-500 mb-1.5 leading-tight'>
                              Reason: {wp.failed_reason}
                            </p>
                          )}
                        </div>
                        <div className='flex flex-col items-end shrink-0 gap-1.5'>
                          <span
                            className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider border ${isFailed ? "bg-red-50 text-red-600 border-red-200" : `bg-${color}-50 text-${color}-600 border-${color}-100`}`}
                          >
      ...
      {isException && (
        <div className='bg-white border-t border-red-100 px-6 py-4 mt-auto'>
          <div className='flex gap-3'>
            <button
              onClick={() => onReturn?.(order.failed_shipments?.[0])}
              className='flex-1 py-2.5 px-4 rounded-xl border-2 border-slate-200 text-slate-700 font-bold text-sm tracking-wide flex items-center justify-center gap-2 hover:bg-slate-50 hover:border-slate-300 transition-colors shadow-sm'
            >
            ...
            <button
              onClick={onAssign}
              className='flex-[1.5] py-2.5 px-4 rounded-xl relative overflow-hidden group border border-red-600 text-white font-bold text-sm tracking-wide shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2'
            >
