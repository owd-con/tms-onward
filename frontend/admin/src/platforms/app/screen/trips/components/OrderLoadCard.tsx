...
export const OrderLoadCard: React.FC<OrderLoadCardProps> = ({
  order,
  isSelected,
  onClick,
  onAssign,
  ...
  // Aggregate data
  const totalWeight =
    order.shipments?.reduce((acc, curr) => acc + (curr.total_weight || 0), 0) ||
    0;

  const uniqueOrigins =
    new Set(order.shipments?.map((s) => s.origin_address_id)).size || 0;
  const uniqueDestinations =
    new Set(order.shipments?.map((s) => s.destination_address_id)).size || 0;
  ...
  const pickupDateTime = formatDateTime(
    firstShipment?.scheduled_pickup_date,
    firstShipment?.scheduled_pickup_time,
  );
  const deliveryDateTime = formatDateTime(
    lastShipment?.scheduled_delivery_date,
    lastShipment?.scheduled_delivery_time,
  );
    ...
    >
      ...
      <div className='flex items-center gap-3 mb-5 px-1'>
        <div className='flex-1 min-w-0'>
          <p className='text-[13px] font-bold text-slate-900 truncate'>
            {originName}
            {originExtra > 0 && (
              <span className='ml-1.5 px-1.5 py-0.5 rounded-md bg-primary/10 text-primary text-[9px] font-black uppercase inline-block align-middle mb-0.5'>
                +{originExtra}
              </span>
            )}
          </p>
        ...
        <div className='flex-1 text-right min-w-0'>
          <p className='text-[13px] font-bold text-slate-900 truncate'>
            {destExtra > 0 && (
              <span className='mr-1.5 px-1.5 py-0.5 rounded-md bg-primary/10 text-primary text-[9px] font-black uppercase inline-block align-middle mb-0.5'>
                +{destExtra}
              </span>
            )}
            {destinationName}
      ...
      <div className='flex bg-slate-50/80 rounded-xl p-2.5 border border-slate-100 mb-5 text-center'>
        ...
        <div className='flex-1 border-r border-slate-200/60 pl-1'>
          ...
          <p className='text-sm font-black text-slate-800 flex items-center justify-center gap-1'>
            <Combine className='size-[14px] text-slate-400' />{" "}
            {uniqueDestinations}
          </p>
        </div>
        <div className='flex-1 pl-1'>
          ...
          <p className='text-sm font-black text-slate-800 flex items-center justify-center gap-1'>
            <Weight className='size-[14px] text-slate-400' /> {totalWeight}{" "}
      ...
      <div className='flex justify-between items-center'>
        <div className='flex items-center gap-2.5'>
          ...
          <div>
            <p className='text-[13px] font-bold text-slate-900 leading-none mb-1 truncate max-w-[100px]'>
              {customerName}
            </p>
