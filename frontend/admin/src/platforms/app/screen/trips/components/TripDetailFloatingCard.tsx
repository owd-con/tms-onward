...
export const TripDetailFloatingCard: React.FC<TripDetailFloatingCardProps> = ({
  load,
  onClose,
}) => {
  const orderNumber =
    load?.order?.order_number || load?.trip_number || "Unknown";
    ...
    <div className='lg:absolute lg:top-6 lg:left-6 w-full lg:w-[400px] bg-white/95 backdrop-blur-md lg:rounded-[24px] rounded-t-[24px] lg:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] shadow-none border-x border-t lg:border-white/60 border-slate-100 overflow-hidden z-20 flex flex-col h-full lg:h-auto lg:max-h-[calc(100vh-48px)]'>
      ...
      <div className='px-6 pt-5 pb-4 flex justify-between items-center bg-white/50 border-b border-slate-100/80'>
        <div className='flex items-center gap-3'>
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-[11px] tracking-widest border shrink-0 ${badgeColors}`}
          >
            {orderType}
          </div>
          <div className='flex flex-col min-w-0'>
            <h2 className='text-[15px] font-bold text-slate-900 tracking-tight leading-none mb-1 truncate'>
              {orderNumber}
            </h2>
          ...
          <div className='p-4 bg-transparent rounded-2xl border border-slate-200 flex gap-4'>
            <div className='flex-1 min-w-0'>
              ...
              <p className='text-[13px] font-bold text-slate-900 truncate'>
                {load?.order?.customer?.name || "Unknown"}
              </p>
            </div>
            ...
            <div className='flex-1'>
              ...
              <p className='text-[13px] font-bold text-slate-900'>
                {load?.order?.total_delivered || 0} /{" "}
                {load?.order?.total_shipment || 0}
              </p>
            </div>
          </div>
          ...
          {isActive ? (
            ...
            <div className='p-4 bg-transparent rounded-2xl border border-slate-200 flex gap-4'>
              <div className='flex-1'>
                ...
                <p className='text-[13px] font-bold text-slate-900'>
                  {load?.started_at
                    ? dayjs(load.started_at).format("DD MMM • HH:mm")
                    : "-"}
                </p>
              </div>
              <div className='w-px bg-slate-200'></div>
              <div className='flex-1 text-right'>
                <p className='text-[10px] font-semibold text-slate-500 mb-1 uppercase tracking-wider text-right'>
                  Finished At
                </p>
                <p className='text-[13px] font-bold text-slate-900'>
                  {load?.completed_at
                    ? dayjs(load.completed_at).format("DD MMM • HH:mm")
                    : "-"}
                </p>
              </div>
            </div>
          ...
          <div className='relative'>
            {waypoints.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      {!isLast &&
                        (() => {
                          const nextIsCompleted =
                            waypoints[idx + 1]?.status === "completed";
                          const fillClass = nextIsCompleted
                            ? "h-full"
                            : isCompleted
                              ? "h-1/2"
                              : "h-0";
                          return (
                            <div className='absolute left-[10px] top-[18px] -bottom-[18px] w-0.5 -translate-x-1/2 bg-slate-200 z-0 overflow-hidden'>
                              <div
                                className={`absolute top-0 left-0 w-full bg-emerald-500 transition-all duration-500 ${fillClass}`}
                              ></div>
                            </div>
                          );
                        })()}
                      ...
                      >
                        <MapPin
                          size={18}
                          className={`text-${color}-500 ${isCompleted || isFailed ? "opacity-100" : "opacity-40"}`}
                          strokeWidth={2.5}
                        />
                      </div>
                      ...
                      <div className='flex justify-between items-start gap-4 pt-1 w-full'>
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-[11px] text-slate-600 mb-1.5 truncate max-w-full'>
                            {wp.address || wp.address_rel?.address}
                          </p>
                          {isFailed && wp.failed_reason && (
                            <p className='text-[10px] font-bold text-rose-500 mb-1.5 leading-tight'>
                        ...
                        <div className='flex flex-col items-end shrink-0 gap-1.5'>
                          ...
                          {wp.actual_completion_time ? (
                            <p className='text-[10px] font-semibold text-slate-400 leading-none'>
                              {dayjs(wp.actual_completion_time).format(
                                "DD MMM • HH:mm",
                              )}
                            </p>
                          ) : (
                            <p className='text-[10px] font-semibold text-slate-400 italic leading-none'>
                              {dayjs(wp.completion_at).format("HH:mm A")}
                            </p>
                          )}
                        </div>
                      </div>
              ...
              <div className='text-center py-8 text-slate-500 text-sm italic'>
                No waypoints defined for this trip.
              </div>
            )}
          </div>
        </div>
      ...
      <div className='bg-white border-t border-slate-100 px-6 py-3.5 mt-auto flex items-center justify-between gap-4'>
        <div className='flex items-center gap-4 flex-1 min-w-0'>
          ...
          <div className='flex items-center gap-2.5 min-w-0 flex-1'>
            ...
            <div className='flex flex-col min-w-0'>
              <h4 className='text-[12px] font-bold text-slate-900 leading-tight truncate'>
                {load?.driver?.name || load?.user?.name || "Unassigned"}
              </h4>
              <p className='text-[10px] text-slate-500 font-medium truncate'>
                {load?.driver?.phone || load?.user?.phone || "No phone"}
              </p>
            </div>
          </div>
          ...
          <div className='flex items-center gap-2.5 min-w-0 flex-1 border-l border-slate-100 pl-4'>
            ...
            <div className='flex flex-col min-w-0'>
              <h4 className='text-[12px] font-bold text-slate-900 leading-tight truncate'>
                {load?.vehicle?.plate_number || "No Vehicle"}
              </h4>
