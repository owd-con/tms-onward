 ...
 * - Start Waypoint button (only for pending + trip in_transit)
...
export const WaypointCard = ({
  ...
  const isClickable = ["completed", "failed", "in_transit"].includes(waypoint.status);
      ...
      <div className="space-y-3 pl-11">
        ...
        {waypoint.actual_arrival_time &&
          !waypoint.actual_completion_time && (
            <p className="typo-tiny text-content-secondary">
              Arrived at{" "}
              {formatDateTime(waypoint.actual_arrival_time)}
            </p>
          )}
      </div>

      {/* Start Waypoint button - Only for pending + trip in_transit */}
      {waypoint.status === "pending" && tripStatus === "in_transit" && (
