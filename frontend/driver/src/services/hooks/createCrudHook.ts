...
export interface CrudHookConfig<TEntity = unknown> {
  ...
  readonly _?: TEntity;
  /**
   * Lazy query hook for listing entities (optional - e.g., upload doesn't need list)
   */
      ...
      if (config.customOperations) {
        for (const [key, operation] of Object.entries(
          ...
          if (requiresId) {
            ...
            // Custom operation doesn't require id (like uploadFile)
            // Using unknown to support File, FormData, and other non-object types
            basicOps[key] = async (payload: unknown) => {
              try {
